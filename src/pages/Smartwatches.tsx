...
export default function Smartwatches() {
  const { t } = useLanguage();
  const { items, brands, brand, setBrand, sort, setSort } = useProductFilters('smartwatches');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');

  return (
    <div className="min-h-screen bg-[#f8fafc]">
      <Navbar />
      <Seo title={t('smartwatches.title')} description={t('smartwatches.subtitle')} />
      <main className="max-w-7xl mx-auto px-4 pt-2 pb-12">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8 font-medium">
          ...
          <ChevronRight size={14} />
          <span className="text-gray-900 font-bold">{t('breadcrumb.smartwatches')}</span>
        </nav>
